...
export default function EditChargeScreen() {
  ...
  const handleSave = () => {
    if (!location.trim() || !charge.trim()) {
      Alert.alert('Error', 'Please fill in both location and charge amount');
      return;
    }
    
    const action = isEditing ? 'updated' : 'added';
    Alert.alert('Success', `Charge has been ${action} successfully`);
