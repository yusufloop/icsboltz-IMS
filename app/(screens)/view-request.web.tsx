              ...
              <View className="px-6 py-6 space-y-6">
                ...
                <View>
                  ...
                  <View className="mb-4">
                    ...
                    <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[44px] ${isFieldEditable('itemRequested') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                      <TextInput className="flex-1 text-base text-text-primary" value={requestData.itemRequested} onChangeText={(text) => isFieldEditable('itemRequested') && updateField('itemRequested', text)} editable={isFieldEditable('itemRequested')} placeholder="Enter product name" placeholderTextColor="#8A8A8E" />
                    </View>
                  </View>
                  <View className="flex-row space-x-4 mb-4">
                    <View className="w-32">
                      <Text className="text-sm font-semibold text-text-primary mb-2">Quantity</Text>
                      <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[44px] ${isFieldEditable('quantity') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                        <TextInput className="flex-1 text-base text-text-primary" value={requestData.quantity} onChangeText={(text) => isFieldEditable('quantity') && updateField('quantity', text)} editable={isFieldEditable('quantity')} keyboardType="numeric" placeholder="Enter quantity" placeholderTextColor="#8A8A8E" />
                      </View>
                    </View>
                    <View className="flex-1">
                      <Text className="text-sm font-semibold text-text-primary mb-2">Phone No</Text>
                      <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[44px] ${isFieldEditable('phoneNo') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                        <TextInput className="flex-1 text-base text-text-primary" value={requestData.phoneNo} onChangeText={(text) => isFieldEditable('phoneNo') && updateField('phoneNo', text)} editable={isFieldEditable('phoneNo')} keyboardType="phone-pad" placeholder="Enter phone Number" placeholderTextColor="#8A8A8E" />
                      </View>
                    </View>
                  </View>
                  <View className="mb-4">
                    <Text className="text-sm font-semibold text-text-primary mb-2">Reason For Request</Text>
                    <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[80px] ${isFieldEditable('reasonForRequest') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                      <TextInput value={requestData.reasonForRequest} onChangeText={(text) => isFieldEditable('reasonForRequest') && updateField('reasonForRequest', text)} editable={isFieldEditable('reasonForRequest')} placeholder="Enter why you requested the item" placeholderTextColor="#8A8A8E" multiline numberOfLines={3} className="flex-1 text-base text-text-primary" textAlignVertical="top" />
                    </View>
                  </View>
                  ...
                  <View className="flex-row space-x-4 mb-4">
                    <View className="flex-1">
                      ...
                      <TouchableOpacity className={`rounded-lg border border-gray-300 flex-row items-center px-4 py-3 min-h-[44px] ${isRequester ? 'bg-gray-100' : 'bg-bg-secondary active:opacity-80'}`} disabled={isRequester}>
                        <MaterialIcons name="calendar-today" size={20} color="#8A8A8E" style={{ marginRight: 12 }} />
                        <Text className="flex-1 text-base text-text-primary">{requestData.dateNeededBy ? formatDate(requestData.dateNeededBy) : 'Date'}</Text>
                      </TouchableOpacity>
                    </View>

                    {/* IMPROVEMENT: Priority dropdown now "pushes" content */}
                    <View className="flex-1">
                      <Text className="text-sm font-semibold text-text-primary mb-2">Priority</Text>
                      <TouchableOpacity 
                        className={`rounded-lg border flex-row items-center px-4 py-3 min-h-[44px] ${isRequester ? 'opacity-60' : 'active:opacity-80'}`}
                        style={{ backgroundColor: priorityInfo.bgColor, borderColor: priorityInfo.color + '40' }}
                        onPress={() => !isRequester && setShowPriorityDropdown(!showPriorityDropdown)}
                        disabled={isRequester}
                      >
                        <Text className="flex-1 text-base font-medium" style={{ color: priorityInfo.color }}>{priorityInfo.label}</Text>
                        {!isRequester && <MaterialIcons name="chevron-right" size={20} color={priorityInfo.color} />}
                      </TouchableOpacity>
                      
                      {/* By removing absolute positioning, this View is now in the layout flow */}
                      {showPriorityDropdown && !isRequester && (
                        <View className="bg-white border border-gray-200 rounded-lg shadow-md mt-2">
                          {PRIORITY_OPTIONS.map((option) => (
                            <TouchableOpacity 
                              key={option.value}
                              onPress={() => { updateField('priority', option.value); setShowPriorityDropdown(false); }}
                              className="px-4 py-3 border-b border-gray-200 last:border-b-0 active:opacity-80"
                            >
                              <Text className="text-base font-medium" style={{ color: option.color }}>{option.label}</Text>
                            </TouchableOpacity>
                          ))}
                        </View>
                      )}
                  ...
                  {/* IMPROVEMENT: Department dropdown now "pushes" content */}
                  <View className="mb-4">
                    <Text className="text-sm font-semibold text-text-primary mb-2">Charge to Department</Text>
                    <TouchableOpacity 
                      className={`rounded-lg border border-gray-300 flex-row items-center px-4 py-3 min-h-[44px] ${isRequester ? 'bg-gray-100' : 'bg-bg-secondary active:opacity-80'}`}
                      onPress={() => !isRequester && setShowDepartmentDropdown(!showDepartmentDropdown)}
                      disabled={isRequester}
                    >
                      <Text className="flex-1 text-base text-text-primary">{requestData.chargeToDepartment || 'Marketing'}</Text>
                      {!isRequester && <MaterialIcons name="unfold-more" size={20} color="#8A8A8E" />}
                    </TouchableOpacity>
                    
                    {/* This dropdown is now part of the layout flow and will push content down */}
                    {showDepartmentDropdown && !isRequester && (
                      <View className="bg-white border border-gray-200 rounded-lg shadow-md mt-2 max-h-48">
                        <ScrollView showsVerticalScrollIndicator={false}>
                          {DEPARTMENT_OPTIONS.map((department) => (
                            <TouchableOpacity 
                              key={department}
                              onPress={() => { updateField('chargeToDepartment', department); setShowDepartmentDropdown(false); }}
                              className="px-4 py-3 border-b border-gray-200 last:border-b-0 active:opacity-80"
                            >
                              <Text className="text-base text-text-primary">{department}</Text>
                            </TouchableOpacity>
                          ))}
                        </ScrollView>
                      </View>
                ...
                <View>
                  ...
                  <View className="flex-row space-x-4 mb-4">
                    <View className="flex-1">
                      ...
                      <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[44px] ${isFieldEditable('estimatedCost') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                        <TextInput className="flex-1 text-base text-text-primary" value={requestData.estimatedCost} onChangeText={(text) => isFieldEditable('estimatedCost') && updateField('estimatedCost', text)} editable={isFieldEditable('estimatedCost')} placeholder="Enter estimated cost" placeholderTextColor="#8A8A8E" />
                      </View>
                    </View>
                    <View className="flex-1">
                      <Text className="text-sm font-semibold text-text-primary mb-2">Budget Code</Text>
                      <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[44px] ${isFieldEditable('budgetCode') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                        <TextInput className="flex-1 text-base text-text-primary" value={requestData.budgetCode} onChangeText={(text) => isFieldEditable('budgetCode') && updateField('budgetCode', text)} editable={isFieldEditable('budgetCode')} placeholder="Enter budget code" placeholderTextColor="#8A8A8E" />
                      </View>
                    </View>
                  </View>

                  {/* IMPROVEMENT: Approval Level dropdown now "pushes" content */}
                  <View className="mb-4">
                    <Text className="text-sm font-semibold text-text-primary mb-2">Approval Level Required</Text>
                    <TouchableOpacity 
                      className={`rounded-lg border border-gray-300 flex-row items-center px-4 py-3 min-h-[44px] ${isFieldEditable('approvalLevel') ? 'bg-bg-secondary active:opacity-80' : 'bg-gray-100'}`}
                      onPress={() => isFieldEditable('approvalLevel') && setShowApprovalLevelDropdown(!showApprovalLevelDropdown)}
                      disabled={!isFieldEditable('approvalLevel')}
                    >
                      <Text className="flex-1 text-base text-text-primary">{requestData.approvalLevel}</Text>
                      {isFieldEditable('approvalLevel') && <MaterialIcons name="unfold-more" size={20} color="#8A8A8E" />}
                    </TouchableOpacity>
                    
                    {/* This dropdown is now part of the layout flow and will push content down */}
                    {showApprovalLevelDropdown && isFieldEditable('approvalLevel') && (
                      <View className="bg-white border border-gray-200 rounded-lg shadow-md mt-2">
                        {APPROVAL_LEVELS.map((level) => (
                          <TouchableOpacity 
                            key={level}
                            onPress={() => { updateField('approvalLevel', level); setShowApprovalLevelDropdown(false); }}
                            className="px-4 py-3 border-b border-gray-200 last:border-b-0 active:opacity-80"
                          >
                ...
                {(requestData.hodComments || requestData.managerComments || canReject) && (
                  <View>
                    ...
                    {requestData.hodComments && (
                      <View className="mb-4">
                        <Text className="text-sm font-semibold text-text-primary mb-2">Head of Department Comments</Text>
                        <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[80px] ${isFieldEditable('hodComments') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                          <TextInput value={requestData.hodComments} onChangeText={(text) => isFieldEditable('hodComments') && updateField('hodComments', text)} editable={isFieldEditable('hodComments')} multiline numberOfLines={3} className="flex-1 text-base text-text-primary" textAlignVertical="top" placeholder="HOD comments..." placeholderTextColor="#8A8A8E" />
                        </View>
                      </View>
                    )}
                    {requestData.managerComments && (
                      <View className="mb-4">
                        <Text className="text-sm font-semibold text-text-primary mb-2">Manager Comments</Text>
                        <View className={`rounded-lg border border-gray-300 px-4 py-3 min-h-[80px] ${isFieldEditable('managerComments') ? 'bg-bg-secondary' : 'bg-gray-100'}`}>
                          <TextInput value={requestData.managerComments} onChangeText={(text) => isFieldEditable('managerComments') && updateField('managerComments', text)} editable={isFieldEditable('managerComments')} multiline numberOfLines={3} className="flex-1 text-base text-text-primary" textAlignVertical="top" placeholder="Manager comments..." placeholderTextColor="#8A8A8E" />
                        </View>
                      </View>
                    )}
                    {canReject && (
                      <View className="mb-4">
                        <Text className="text-sm font-semibold text-text-primary mb-2">Reason for Rejection <Text className="text-red-500">*</Text></Text>
                        <View className="rounded-lg bg-bg-secondary border border-gray-300 px-4 py-3 min-h-[100px]">
                          <TextInput value={requestData.rejectionReason} onChangeText={(text) => updateField('rejectionReason', text)} placeholder="Please provide a detailed reason for rejection..." placeholderTextColor="#8A8A8E" multiline numberOfLines={4} className="flex-1 text-base text-text-primary" textAlignVertical="top" />
                        </View>
                      </View>
                    )}
                  </View>
                ...
                <View>
                  ...
                  {!isRequester && (
                    <TouchableOpacity className="border-2 border-dashed rounded-lg p-8 items-center border-gray-300 bg-gray-50" onPress={handleFileUpload}>
                      ...
                      <Text className="text-gray-500 text-center mb-2">Drag File/Image here</Text>
              ...
              {(canApprove || canReject) && (
                <View className="border-t border-gray-200 px-6 py-4 bg-white">
                  <View className="flex-row space-x-4">
                    {canReject && (
                      <TouchableOpacity onPress={handleReject} className="flex-1 bg-red-500 border border-red-600 rounded-lg px-4 py-3 min-h-[44px] items-center justify-center active:opacity-80">
                        <Text className="text-base font-semibold text-white">Reject</Text>
                      </TouchableOpacity>
                    )}
                    {canApprove && (
                      <TouchableOpacity onPress={handleApprove} className="flex-1 bg-blue-500 border border-blue-600 rounded-lg px-4 py-3 min-h-[44px] items-center justify-center active:opacity-80">
