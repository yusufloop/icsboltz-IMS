...
// UPDATE: Added 'comments' to the form data
interface ResubmitRequestFormData {
  ...
  attachments: any[];
  // NEW: Field for resubmission
  comments: string;
}

// UPDATE: Props now accept initial data to pre-fill the form
interface ResubmitRequestModalProps {
  visible: boolean;
  onClose: () => void;
  onSubmit: (requestData: ResubmitRequestFormData) => void;
  initialData: ResubmitRequestFormData;
}
...
// --- MAIN COMPONENT ---
// UPDATE: Component name changed
export function ResubmitRequestModal({ visible, onClose, onSubmit, initialData }: ResubmitRequestModalProps) {
  // UPDATE: State now uses the new interface
  const [formData, setFormData] = useState<ResubmitRequestFormData>(initialData);

  const [showDatePicker, setShowDatePicker] = useState(false);
  const [showPriorityPicker, setShowPriorityPicker] = useState(false);
  const [showDepartmentPicker, setShowDepartmentPicker] = useState(false);

  // UPDATE: useEffect to update form data if initialData changes while modal is open
  useEffect(() => {
    if (visible) {
  ...
  // --- HANDLER FUNCTIONS ---
  const updateField = (field: keyof ResubmitRequestFormData, value: any) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };
  
  // (handleDateChange, handleFileUpload are the same)
        ...
        >
          ...
          >
            ...
            >
              {/*
              ===============================================================
              NEW FIELD: Comments from Head of Department
              ===============================================================
              */}
              <View>
                ...
                <View className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                  <Text className="text-base text-yellow-800 font-system italic">
                    "The initial quantity requested seems too high for the current budget cycle. Please review and adjust the amount or provide additional justification."
                  </Text>
                </View>
              </View>
              ...
              {/* ... (Rest of the form fields are identical to NewRequestModal) ... */}
          ...
          >
            {/* UPDATE: Button title changed */}
            <PremiumButton title="Resubmit" onPress={handleSubmit} variant="gradient" size="lg" />
          </Animated.View>

          {/* PICKER MODALS (No changes needed) */}
